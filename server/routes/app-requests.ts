...
router.post('/', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO app_requests (id, problemDescription, status, submittedAt) VALUES ($1, $2, $3, $4);', [newRequest.id, newRequest.problemDescription, newRequest.status, newRequest.submittedAt]);
...
router.patch('/', async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE app_requests SET status = $1 WHERE id = $2 RETURNING *;', [status, id]);
